...
export default function Pricing() {
  return (
    <section id="pricing" className="py-12 md:py-16 bg-[#FDFCFB] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 lg:gap-12">
          {services.map((service, index) => (
            <motion.div
              ...
              className="bg-white border border-gold/30 p-5 md:p-6 rounded-sm shadow-sm flex flex-col hover:border-gold/60 transition-colors"
            >
              <div className="flex flex-col sm:flex-row justify-between sm:items-end border-b border-dashed border-gray-200 pb-3 mb-3 gap-2">
                ...
                <span className="font-serif font-bold text-gold text-lg md:text-xl shrink-0 sm:text-right pl-11 sm:pl-0">
                  {service.price}
                </span>
              </div>
              <p className="text-sm md:text-base text-[#4D4741] font-medium leading-relaxed pl-0 sm:pl-11">
                {service.description}
              </p>
            </motion.div>
          ))}
        ...
        >
          <p className="text-xs md:text-sm font-medium italic text-[#4D4741]">
